...
class TD3(object):
    ...
    def train(self, replay_buffer, batch_size=100, INVASE_PVS = None):
        self.total_it += 1
        
        # Sample replay buffer 
        state, action, next_state, reward, not_done = replay_buffer.sample(batch_size)
        if INVASE_PVS is None:
            with torch.no_grad():
        ...
        else: # training with PVS network (generator)
            '''
            ...
            with torch.no_grad():
                ...
                sel_prob_next_action = INVASE_PVS.generator(next_state,next_action)
            ...
            sel_prob_action = INVASE_PVS.generator(state,action)
            sel_prob_action = 1.*(sel_prob_action > 0.5)
            '''G dot action'''
            ...
            if self.total_it % self.policy_freq == 0:
                ...
                sel_prob_actor_action = INVASE_PVS.generator(state, action_pred_by_actor.detach())
